...
# -*- coding: utf-8 -*-
"""
map_qubit_left_v2_gpu — version hybride GPU-friendly de map_qubit_left_v2_unified

Remarques :
- Détecte cupy (GPU) et utilise le GPU pour des opérations vectorielles/interpolation
  quand c'est possible. QuTiP / solveurs d'autovecteurs externes restent en CPU.
- Si cupy est trouvé, on désactive le multi-processing pour éviter la contention GPU.
- Garder les memmap (.npy) en numpy (format sur disque).
"""
...
# Essentiels du projet (inchangés)
...
FORCE_RECALC        = False
# Si GPU actif, on ne lance PAS de process parallèles qui se partageraient le GPU.
USE_PARALLEL        = True if not USE_GPU else False
# Par défaut : cpu_count()-1 ; si GPU activé on force 1 worker
MAX_WORKERS         = max(1, (os.cpu_count() or 2) - 1) if not USE_GPU else 1
ESTIMATE_RUNTIME    = True
BENCH_FRAC_DT       = 0.10
BENCH_MIN_SAMPLES   = 20
UPSAMPLE_TO_HIGHRES = False

print(f"[INFO] USE_GPU={USE_GPU}  |  os.cpu_count()={os.cpu_count()}  |  MAX_WORKERS={MAX_WORKERS}  |  USE_PARALLEL={USE_PARALLEL}")
...
def qubits_impulsion_lastonly(num_sites, n_electrons,
    ...
    ATTENTION : QuTiP reste sur CPU — pas d'accélération GPU ici.
    """
...
def compute_txU_for_pulse(delta_t, delta_U_meV, imp_start_idx):
    ...
    # potential_over_time est probablement CPU (numpy). On conserve.
    ...
    # États propres + localisation : restent CPU (scipy/quasi)
...
def build_p_nochange_rows(delta_U_vals, delta_t_vals, phi0, base_dir,
    ...
    if USE_PARALLEL and len(todo) > 1:
        # Parallélisation CPU : chaque worker calcule une ligne ΔU entière (inchangé)
        ...
        # mode séquentiel (ou GPU single-process)
...
def interpolate_to_full(coarse_map, Uc, Tc, Uf, Tf):
    """Interpolation ; tente une version GPU via cupy si disponible."""
    if USE_GPU and cp is not None:
        try:
            # passer en cupy
            cm = cp.asarray(coarse_map, dtype=cp.float32)
            # interpolation bilinéaire simple sur GPU (pas RectBivariateSpline)
            # on fait d'abord interp en T pour chaque U (1D interp via searchsorted)
            Tf_cpu = np.asarray(Tf)
            Tc_cpu = np.asarray(Tc)
            Uf_cpu = np.asarray(Uf)
            # on utilise numpy.interp parce que cupy n'a pas d'équivalent direct pour searchsorted+interp vectorisé simple
            # stratégie : transférer les vecteurs 1D Tc,Tf, puis pour chaque i faire interp sur CPU est inefficace.
            # solution simple et robuste : faire interpolation séquentielle sur CPU si nécessaire.
            # (si tu veux 100% GPU pour l'interp, il faut implementer un kernel custom)
            return _interp_cpu_fallback(coarse_map, Uc, Tc, Uf, Tf)
        except Exception:
            return _interp_cpu_fallback(coarse_map, Uc, Tc, Uf, Tf)
    else:
        return _interp_cpu_fallback(coarse_map, Uc, Tc, Uf, Tf)

def _interp_cpu_fallback(coarse_map, Uc, Tc, Uf, Tf):
    try:
...
def main_qubit_left(delta_U_vals_full, delta_t_vals_full):
    print("🚀 Démarrage map_qubit_left (GPU-friendly pipeline)")
